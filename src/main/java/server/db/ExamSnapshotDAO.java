...
public class ExamSnapshotDAO {
 public boolean createForRelease(int releaseId, Exam exam, QuestionDAO questions){Transaction tx=null; try(Session s=HibernateUtil.getSessionFactory().openSession()){tx=s.beginTransaction(); for(ExamQuestion link:exam.getQuestions()){Question q=questions.getById(link.getQuestionId()); if(q==null) throw new IllegalStateException("Missing question "+link.getQuestionId()); s.createNativeMutationQuery("INSERT INTO ReleasedExamQuestions(release_id,question_id,points,position,question_text,answer_1,answer_2,answer_3,answer_4,correct_answer,image_path) VALUES(:r,:q,:p,:pos,:t,:a1,:a2,:a3,:a4,:c,:img)").setParameter("r",releaseId).setParameter("q",q.getId()).setParameter("p",link.getPoints()).setParameter("pos",link.getPosition()).setParameter("t",q.getQuestionText()).setParameter("a1",q.getAnswer1()).setParameter("a2",q.getAnswer2()).setParameter("a3",q.getAnswer3()).setParameter("a4",q.getAnswer4()).setParameter("c",q.getCorrectAnswer()).setParameter("img",q.getImagePath()).executeUpdate();} tx.commit(); return true;}catch(Exception e){if(tx!=null)tx.rollback();System.err.println("[ExamSnapshotDAO] create failed: "+e.getMessage());return false;}}
